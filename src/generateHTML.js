function generateCard(data) {
  const cards = data.map(card => {
    return     `<div class='col-4 card bg-light m-4 p-0 shadow-lg' style='width: 19rem; min-width: 19rem; max-width: 19rem;'>
                  <div class='card h-100'>
                    ...
                    <div class='card-body'>
                      ...
                      <p class='email'>Email:<a href='mailto:${card.getEmail()}'>${card.getEmail()}</a></p>
                        ${(card.getRole() === 'Manager' ? `<p class ='specInfo'>Office Number: ${card.getOfficeN()}</p>` : '') || (card.getRole() === 'Engineer' ? `<p class ='specInfo'>Github: <a href='http://www.github.com/${card.getGithub()}' target='_blank'>${card.getGithub()}</a></p>` : '') || (card.getRole() === 'Intern' ? `<p class ='specInfo'>School: ${card.getSchool()}</p>`: '')}
                    </div>
                  </div>
                </div>`;
  });
        ...
        <main>
          <section class='row justify-content-center p-2' id='infoContainer'>
            <div class='header text-center' id='teamMemberContainer'>
              <h2>Employees</h2>
                <div class='row ms-1 justify-content-center ml-5 mr-5' id='card-container'>
                  ${generateCard(data)}
              </div>
            </div>
          </section>
